...
import com.example.PAYMENT.wallet.AddBalanceWalletRequest
import com.example.PAYMENT.wallet.CreateWalletRequest
import com.example.PAYMENT.wallet.Wallet
import com.example.PAYMENT.wallet.WalletRepository
...
class WalletServiceSpockTest extends Specification{
    ...
    def "지갑 생성 요청 시 지갑을 갖고 있지 않다면 생성된다"() {
        given:
    ...
    def "지갑 생성 요청시 지갑을 갖고 있다면 오류를 던진다"() {
        given:
        CreateWalletRequest request = new CreateWalletRequest(1L)
    ...
    def "지갑 잔액 추가 시 지갑이 존재하지 않으면 예외를 던진다"() {
        given:
        def walletId = 999L
        def addAmount = new BigDecimal("100.00")
    ...
    def "지갑 잔액 추가 시 잔액이 부족하면 예외를 던진다"() {
        given:
        def walletId = 1L
    ...
    def "지갑 잔액 추가 시 한도를 초과하면 예외를 던진다"() {
        given:
        def walletId = 1L
